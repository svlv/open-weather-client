...
int main(int argc, char *argv[]) {
  ...
  desc.add_options()("help", "Produce help message")(
      "get-current-weather",
      "Get current weather for your location on Earth (London is default)")(
      "get-current-weather-with-pollution",
      "Get current weather and air pollution")("get-forecast",
                                               "Get 5 day/3 hour forecast")(
      "lat", po::value<double>(&lat)->default_value(51.5072, "51.5072"),
      "Geographical coordinate - latitude")(
      "lon", po::value<double>(&lon)->default_value(0.1276, "0.1276"),
      "Geographical coordinate - longitude")(
      "token",
      po::value<std::string>(&token)->default_value(
          "0dcf521f731dd46db31533ed4494c04c"),
      "Your unique API key");
  ...
  if (vm.count("get-current-weather-with-pollution")) {
    ...
    printf("%s %d°C %dm/s %s", weather.weather[0].main.c_str(), cels,
           wind_speed, get_aqi_name(aqi).data());
  }
  ...
  if (vm.count("get-forecast")) {
    ...
    for (const auto &elem : data.list) {
      ...
      sprintf(lines[i], "%-6s %3d°C %2dm/s", elem.weather[0].main.c_str(), cels,
              wind_speed);
      lines[i][21] = '\0';
      ++i;
    }
